...
/**
 * 「システムロード」コマンドを解析するローダーです。
 * 
 * @author hide92795
 */
public class CommandSystemLoad extends Command {
	@Override
	public void save(StreamTokenizer tokenizer, LinkedList<Object> commandLine) throws Exception {
		// int ファイル番号
		commandLine.add(SaverStory.COMMAND_SYSTEM_LOAD);
		int next = tokenizer.nextToken();
		if (next != StreamTokenizer.TT_NUMBER) {
			// 非数値
			throw new CommandException(tokenizer.lineno(), "システムロード", 1, "引数「ファイル番号」は数値でなければいけません。");
		} else {
		...
		if (next != SaverStory.ARGUMENT_END) {
			throw new CommandException(tokenizer.lineno(), "システムロード", -1, "引数が閉じられていません。");
		}
	}
}
