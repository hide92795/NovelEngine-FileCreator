...
/**
 * 「BGM再生」コマンドを解析するローダーです。
 * 
 * @author hide92795
 */
public class CommandPlayBGM extends Command {
	@Override
	public void save(StreamTokenizer tokenizer, LinkedList<Object> commandLine) throws Exception {
		// String サウンドID
		commandLine.add(SaverStory.COMMAND_PLAY_BGM);
		int next = tokenizer.nextToken();
		if (next != SaverStory.DOUBLE_QUOTE) {
			// 非文字
			throw new CommandException(tokenizer.lineno(), "BGM再生", 0, "引数「サウンドID」は文字列でなければいけません。");
		} else {
		...
		if (next != SaverStory.ARGUMENT_END) {
			throw new CommandException(tokenizer.lineno(), "BGM再生", -1, "引数が閉じられていません。");
		}
	}
}
